...
async function getTagList() {
  loader.value = true;
  const res = await axios.get(`${url}/api/getTagList?type=virtualsport`);
  tags.value = res.data;
  tags.value = ["all", ...tags.value];
}

async function getProvidersList() {
  const res = await axios.get(`${url}/api/getProviderList?type=virtualsport`);
...
async function getGames(tag) {
  ...
  const res = await axios.get(
    `${url}/api/getGames?type=virtualsport&page=${page.value}&tag=${activeTag.value}&provider=${activeProvider.value.provider}`
  );
...
<template>
  <div class="h-screen overscroll-y-auto">
    ...
    />
    <GamesList :games="games" type="virtualsport" />
